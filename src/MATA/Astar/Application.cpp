...
Application::Application(ui _beam_size, const char *lower_bound) {
	q_n = g_n = 0;
	...
	verify_upper_bound = INF;				// verify_upper_bound 就是论文中的tau，graph similarty search的阈值；在这里默认设置为INF.
	upper_bound = verify_upper_bound + 1;					// upper_bound：在GED计算中的上界，剪枝搜索的
...
//
void Application::init(const Graph *g, const Graph *q, int* ub_map, int* _MO, int**_matched_nodes, int _k) {
	q_n = q->n;
	...
	// init matched nodes
	MO = new int[q_n];
	memcpy(MO, _MO, sizeof(int)*q_n);

	if (_k == -1 || _k == -2) {
		k = q_n;		// k ==-1 or -2: select all nodes of g as topk
		in_matched_nodes = new bool*[q_n];
		for (int i = 0; i < q_n; i++) {
			bool *b = new bool[g_n];
			memset(b, true, sizeof(bool)*g_n);
			in_matched_nodes[i] = b;
		}
		...
		for (int i = 0; i < q_n; i++) {
			bool *b = new bool[g_n];
			memset(b, false, sizeof(bool)*g_n);
			in_matched_nodes[i] = b;
		}
		for (int i = 0; i < q_n; i++) {
			for (int j = 0; j < k; j++) {
				int v = _matched_nodes[i][j];
				in_matched_nodes[i][v] = true;
			}
		}
	}
	// update upper bound
	upper_bound = compute_ub_map(ub_map);
}
...
void Application::preprocess() {
	...
	vlabels_n = relabel(q_n, q_vlabels, g_n, g_vlabels);					// 节点标签个数
	elabels_n = relabel(q_starts[q_n], q_elabels, g_starts[g_n], g_elabels);// 边标签个数
	...
	if (q_matrix != NULL) delete[] q_matrix;					// TODO 这个不太理解
...
// 将graph pair的中所有的节点标签重新赋予一个新的连续标签
// array1, array2, 分别表示原始标签数组
...
// 根据 新的标签，为graph pair中的节点标签，找到新的标签。
...
ui Application::AStar(vector<pair<ui, ui> > *mapping_ptr, int *lb_ptr) {
	...
	MO = NULL;
	if (MO == NULL) compute_mapping_order();
	vector<int> my_mo = { 1,2,8,0,4,10,5,9,3,7,6,11 };
	for (int i = 0; i < q_n; i++) my_mo[i] = i;
	unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
	shuffle(my_mo.begin(), my_mo.end(), std::default_random_engine(seed));
	...
	if (q_n > 1 && MO[0] == 1 && MO[1] == 1) {					// 1： 随机生成Matching Order
		vector<int> my_mo;
		for (int i = 0; i < q_n; i++) my_mo.push_back(i);
		unsigned seed = std::chrono::system_clock::now().time_since_epoch().count();
		shuffle(my_mo.begin(), my_mo.end(), std::default_random_engine(seed));
	}
	else if (q_n > 1 && MO[0] == 0 && MO[1] == 0) {				// 0: 使用LSa中的Matching Order
		MO = NULL;
		compute_mapping_order();
	}
	State full;
	search_n = q_n;
	...
	generate_best_extension(NULL, root);		// 计算lb？ 这个是核心操作
	vector<State*> heap;						// 存储所有匹配节点集合
	ui heap_n = 1;
	heap.pb(root);
	...
	while (heap_n > 0 && heap[0]->lower_bound < upper_bound && (verify_upper_bound == INF || upper_bound > verify_upper_bound)) {
		if (heap_n > beam_size) {
			/*int max_cost = heap[0]->lower_bound, max_cost_idx = 0;
			for (int i = 0; i < heap_n; i++) {
				if (heap[i]->lower_bound > max_cost) { max_cost = heap[i]->lower_bound; max_cost_idx = i;}
			}
			State *t = heap[max_cost_idx];
			heap[max_cost_idx] = heap[heap_n - 1];
			heap[heap_n - 1] = t;
			heap_n--;
			heap_bottom_up(max_cost_idx, heap);*/
			heap_n--;
		}
		...
		//std::cout << "Astar node map level, " << now->level << " node, image:" << MO[now->level]<< "->" << now->image << std::endl;
		...
		if (sibling->image < g_n&&sibling->lower_bound < upper_bound) {		// 直观的剪枝操作，
			add_to_heap(sibling, heap_n, heap);								// 把当前点sibling添加到 heap中
			//std::cout << "Astar sibling:" << sibling->level << "->" << sibling->image << std::endl;
		...
		// 找到了一个完整的映射之后，会更新一下上界 upper_bound
		if (now->level + 1 == search_n) extend_to_full_mapping(now, &full);
			...
			now->cs_cnt++;
			//std::cout << "Astar child:" << child->level << "->" << child->image << std::endl;
			if (child->image < g_n&&child->lower_bound < upper_bound) add_to_heap(child, heap_n, heap);	// 把当前匹配的节点添加到 heap中
			else {
		...
		// 根据找到匹配上界 upper_boud，对候选的匹配节点进行剪枝。
...
// 生成一个新的state
// 为了避免一次只生成一个状态，会生成1024个状态放在状态池里，然后在状态池中得到一个状态。
...
// 计算每个节点的 infrequence的子图，不频繁出现的先匹配
// 这里的不频繁包括了节点和边标签的不频繁
void Application::compute_mapping_order() {
	if (MO == NULL) MO = new int[q_n];
...
void Application::print_heap(ui heap_n, vector<State*> &heap) {
	std::cout << " print heap (";
	for (int i = 0; i < heap_n; i++) {
		cout << "[" << MO[heap[i]->level] << "," << heap[i]->image << "," << heap[i]->lower_bound << "] ";
	}
	std::cout << " )" << std::endl;;
}

// 从已经匹配的节点中找到
int Application::find_first_match(std::pair<int, int>* children, int size, bool *cur_matched_nodes)
{
	int idx = -1;
	for (int i = size - 1; i >= 0; i--)  if (cur_matched_nodes[children[i].second] == true) {
		idx = i;
		break;
	}
	return idx;
...
void Application::generate_best_extension(State *parent, State *now) {
	...
	ui candidates_n = 0;
	//bool *cur_matched_nodes = in_matched_nodes[MO[now->level]];
	for (ui i = 0; i < g_n; i++) {
		if (!visY[i]) candidates[candidates_n++] = i;
		else visY[i] = 0;
	}

	//if (candidates_n == 0) {		// 剪枝不可能的搜索
	//	now->lower_bound = INF + 1;
	//	now->siblings_n = 0;
	//	parent->lower_bound = INF + 1;
	//	parent->cs_cnt = 0;
	//	return;
	//}
...
void Application::compute_best_extension_LSa(State *now, ui n, ui *candidates, ui pre_siblings) {
	...
	ui *sizeX = queue;									// sizeX：q中已经匹配的节点induce graph的度数
	ui *sizeY = prev;

	ui idx = now->level;								// idx: 表示当前q中处理的第多少个元素
	ui u = MO[idx], u_cross_cnt = 0, u_inner_cnt = 0;   // u: 表示q中当前待处理的节点;  u_cross_cnt: 节点u的cross edge的个数;  u_inner_cnt:节点u的inneer edge个数
	for (ui i = 0; i <= idx; i++) visX[MO[i]] = 1;
	for (ui i = 0; i <= idx; i++) {						// TODO
		ui v = MO[i];
		sizeX[v] = 0;
	...
	for (ui i = q_starts[u]; i < q_starts[u + 1]; i++) if (visX[q_edges[i]]) ++u_cross_cnt;			// 节点u的cross edge的个数
	...
	for (ui i = idx + 1; i < q_n; i++) --vlabels_map[q_vlabels[MO[i]]];				// q中未匹配的子图
	for (ui i = 0; i <= idx; i++) visX[MO[i]] = 0;

	for (State *parent = now->parent; parent != NULL; parent = parent->parent) {	// visY表示g中已匹配的节点；my表示g和q中节点映射关系
		visY[parent->image] = 1;
	...
	// 找到第一个待匹配的节点，如果找不到，则将当前设置无法匹配
	bool *cur_matched_nodes = in_matched_nodes[MO[now->level]];
	int match_idx = find_first_match(children, n - pre_siblings, cur_matched_nodes);
	if (match_idx == -1) {
		now->image = children[n - pre_siblings - 1].second;
		now->lower_bound = -children[n - pre_siblings - 1].first;
	}
	else {
		now->image = children[match_idx].second;
		now->lower_bound = -children[match_idx].first;
	}
	...
	if (now->lower_bound >= upper_bound || now->lower_bound > verify_upper_bound || match_idx == -1) {
		now->image = g_n;
		return;
	}

	// 把当前层的其他节点 和对应的lower bound添加到 作为兄弟节点添加到 sibliings中

	ushort *t_array = now->siblings;
	//std::cout << "LSa, node->image" << MO[now->level] << "->" << now->image << std::endl;
	//std::cout << "LSa, siblings";
	now->siblings_n = 0;
	for (ui i = 0; i < n - pre_siblings - 1; i++) {
		if (cur_matched_nodes[children[i].second]) {
			t_array[now->siblings_n++] = children[i].second;
			t_array[now->siblings_n++] = -children[i].first;
		}
		//std::cout << children[i].second << ", ";
	}
	//std::cout << endl;
}
...
// 构建匹配的兄弟节点
// 会在同一层中，找到没有被匹配的节点的另一个edit cost最小的点
...
int* Application::compute_operation_of_BX() {
	// if (upper_bound > verify_upper_bound) return upper_bound;
	int* res = new int[5]; // insert node, insert edge, remove edge, relabel
	ui insert_node = 0, insert_edge = 0, remove_edge = 0, relabel = 0, common_edge = 0;
	ui *py = new ui[g_n];
	for (ui i = 0; i < g_n; i++) py[i] = q_n;
	for (ui i = 0; i < q_n; i++)  py[BX[i]] = i;
	ui ged = g_n - q_n;
	...
	for (ui i = 0; i < g_n; i++) for (ui j = g_starts[i]; j < g_starts[i + 1]; j++) {
		...
		++ged;
		if (py[i] >= q_n || py[v] >= q_n) continue;
		if (q_matrix[py[i] * q_n + py[v]] == g_elabels[j]) { ged -= 2; common_edge++; }
		else if (q_matrix[py[i] * q_n + py[v]] < elabels_n) { --ged; }
	}
	delete[] py; py = NULL;
	...
	res[0] = ged;
	res[1] = insert_node;
	res[2] = q_starts[q_n] / 2 - common_edge;
	res[3] = g_starts[g_n] / 2 - common_edge;
	res[4] = relabel;
	return res;
}


// 这里的BX表示，较小图与较大图之间的映射。
bool Application::get_mapping(vector<pair<ui, ui> > &mapping, ui n) {
	bool isSuccess = true;
	mapping.clear();
	for (ui i = 0; i < n; i++) {
		if (BX[i] == -1) {
...
ui Application::compute_ub_map(int * ub_map)
{

	if (q_n > 1 && ub_map[0] == 0 && ub_map[1] == 0) {			// ub_map: 0 表示使用剪枝策略
		return INF + 1;
	}

	ui *py = new ui[g_n];
	for (ui i = 0; i < g_n; i++) py[i] = q_n;
	for (ui i = 0; i < q_n; i++) py[ub_map[i]] = i;

	ui ged = g_n - q_n;
	for (ui i = 0; i < q_n; i++) if (q_vlabels[i] != g_vlabels[ub_map[i]]) ++ged;
