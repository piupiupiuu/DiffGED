...
def transfer_to_torch(unprocessed_u, unprocessed_v, u, v):
    ...
    new_data.x = torch.cat([features_1,features_2],dim=0)
    new_data.edge_index = torch.cat([edges_1,edges_2+features_1.shape[0]],dim=1)
    new_data.x_indicator = torch.cat([torch.zeros((features_1.shape[0],1)),torch.ones((features_2.shape[0],1))],dim=0)
    ...
    '''
    return Batch().from_data_list([new_data]).to('cuda:0')
